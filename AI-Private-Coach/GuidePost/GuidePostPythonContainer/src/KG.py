...
from src.config import neo4j_credentials
...
# --------------- Scoring definitions (formerly in prompts.py) ---------------

COACHING_SCORE_DEFINITION = """Normalized Conversational Balance Score: Score Range	Interpretation
85–100	Excellent balance – speakers are sharing turns and words fairly evenly, few interruptions. Highly effective communication.
70–85	Good balance – some minor imbalances, but conversation is generally fair. Could improve slightly.
50–70	Moderate balance – noticeable domination by one or more speakers, some interruptions. Could affect engagement.
30–50	Poor balance – one speaker dominates or frequent interruptions. Engagement and clarity likely suffer.
0–30	Very poor balance – conversation is heavily dominated or chaotic. High risk of miscommunication or disengagement.
"""

SENTIMENT_SCORE_DEFINITION = """sentimentAnalysis
Score	Interpretation
0–20	Destructive conflict
21–40	Escalating / unresolved
41–60	Neutral / stagnant
61–80	Productive conflict
81–100	Highly constructive resolution
"""

VOLATILITY_SCORE_DEFINITION = """Speaker volatility
Value	Meaning
0–10	Very stable
10–20	Normal emotional fluctuation
20–35	Tense / unstable
35+	Highly volatile conflict"""


def getSystemPrompt(calculated_metrics: str) -> str:
    """Build the CRI/CEI diagnostic scoring prompt with pre-computed metrics baked in."""
    return f"""
  ...
  ### TASK 1: QUANTITATIVE DIAGNOSTIC SCORING (CRI & CEI)
  Evaluate the dialogue dynamics. Using the provided metrics as inputs, determine an overall CRI score (1–5) and CEI score (1–5), each with a brief justification.
  The following quantitative metrics are fixed and must be used exactly as given. Use the provided metrics as fixed inputs. Do not alter their values: {calculated_metrics.replace("{", "{{").replace("}", "}}")}

  #### 1. CRI™ (Conflict Resolution Index)
  - sentimentAnalysis: number from metrics above and given description based on description above
  - Speaker volatility: number from metrics above and given description based on description above
  - Resolution_Progress (LLM generated): Evaluate solution_stage and accountability. Weight solution_stage more heavily than accountability when forming this score.
  - Highly penalize rude or mean language

  #### 2. CEI™ (Communication Effectiveness Index)
  - Turn percentages: number from metrics above
  - Word percentages: number from metrics above
  - Interruption percentage: number from metrics above
  - Normalized Conversational Balance Score: number from metrics above and given description based on description above
  - Alignment (LLM generated): Evaluate qa_completion and convergence equally when forming this score.
  - Highly penalize rude or mean language
  ...
  Do not add any additional sections, commentary, or markdown outside the specified format.
  Do not explain your reasoning outside the CRI/CEI summary lines.
  The overall CRI and CEI scores must logically align with their component signals.
  If component metrics indicate low performance, the overall score cannot be high.
  If component metrics indicate strong performance, the overall score cannot be low.
  Do not be afraid to give 1s if the lanauge used is harsh or 5s if the conversation is very pleasent
  CRI:
  1 — No resolution. Escalation, defensiveness, destructive tone.
  2 — Minimal movement. Issues acknowledged but no progress.
  3 — Some constructive exchange. Partial clarity or accountability.
  4 — Clear movement toward resolution. Shared understanding forming.
  5 — Strong resolution. Accountability, alignment, and clear next steps.
  CEI:
  1 — Chaotic, interruptive, dominated.
  2 — Imbalanced, weak listening, poor clarity.
  3 — Functional but inconsistent alignment.
  4 — Balanced and generally effective.
  5 — Highly aligned, structured, and productive.
  ---
  [CRI/CEI ANALYSIS]
  - CRI Score: Based on the metrics reported in 1. CRI™ (Conflict Resolution Index) - give a small summary and then using model judgement give an overall score from 1 to 5 incrementing by 1
  - CEI Score: Based on the metrics reported in 2. CEI™ (Communication Effectiveness Index) - give a small summary and then using model judgement give an overall score from 1 to 5 incrementing by 1
  - Bottleneck: (Describe the main coaching challenge and its trend)
  ---
  [SEARCH QUERIES]
...
from src.main import getCurrentUserId
import re
...
class PineconeVectorStore:
    ...
    class _Doc:
        ...
        def __init__(self, content: str, metadata: dict):
            """Hold retrieved text and Pinecone metadata in a Document-compatible shape."""
            self.page_content = content
            self.metadata     = metadata

    def __init__(self, index, embedding, namespace: str):
        """Bind a Pinecone index, embedding model, and namespace for add/search calls."""
        self._index     = index
...
class RAGQuery:
    def __init__(self, 
        ...
        """
        Set up embeddings, Pinecone (index plus document and memory namespaces), Cohere prompt chains, and optional Neo4j.
        """
        ...
        # Initialize embedding model: disable HF meta / empty-shell init (transformers 4.40+),
        # which triggers "Cannot copy out of meta tensor; no data!" on .to() or first encode().
        ...
        # COHERE_MODEL: use "command-r-08-2024" for faster responses; "command-r-plus-08-2024" for higher quality
        cohere_model = (os.getenv("COHERE_MODEL") or "command-r-plus-08-2024").strip().strip('"').strip("'")
    ...
    def run_trend_analysis(self, opening, middle, closing, segments = "", user_input="None"):
        ...
        # Persist scores to DB (best-effort — app works without Postgres)
        try:
            cri_match = re.search(r"CRI[^0-9]*([0-9]+(?:\.[0-9]+)?)", full_report, re.IGNORECASE)
            cei_match = re.search(r"CEI[^0-9]*([0-9]+(?:\.[0-9]+)?)", full_report, re.IGNORECASE)

            cri = float(cri_match.group(1)) if cri_match else -1
            cei = float(cei_match.group(1)) if cei_match else -1
            ...
            logger.warning("Could not save session data to DB: %s", db_err)
    ...
    def retrieve_from_neo4j_optimised(self, query: str, top_k: int = 5) -> str:
        ...
        def _run(cypher, params=None):
            """Execute Cypher via ``_run_neo4j_query`` and swallow errors into an empty list."""
            try:
            ...
            for m in matches:
                ...
                if not source_name:
                    ...
                    try:
                        ...
                        base_dir = Path(__file__).resolve().parent.parent / "chunk_texts"
                        chunks_path = base_dir / f"{source_name}_chunks.json"
                        texts_path  = base_dir / f"{source_name}_texts.json"

                        loaded = None
                        if chunks_path.exists():
                            with chunks_path.open("r", encoding="utf-8") as f:
                                loaded = json.load(f)
                        elif texts_path.exists():
                            with texts_path.open("r", encoding="utf-8") as f:
                                loaded = json.load(f)

                        text = ""

                        # New format: list of chunk objects with numeric chunk_id
                        if isinstance(loaded, list):
                            chunk_id = None
                            if isinstance(vid, str) and "_chunk_" in vid:
                                try:
                                    chunk_id = int(vid.rsplit("_chunk_", 1)[1])
                                except ValueError:
                                    chunk_id = None
                            ...
                            if chunk_id is not None:
                                for ch in loaded:
                                    if ch.get("chunk_id") == chunk_id:
                                        ...
                                        break
                        # Backward-compat: old dict format {vector_id: text}
                        elif isinstance(loaded, dict):
                            text = str(loaded.get(vid, "")).strip()

                        # If still empty, fall back to whatever was stored in vector metadata
                        if not text:
                            text = (m.metadata.get("text") or "").strip()
                    except FileNotFoundError:
                        text = (m.metadata.get("text") or "").strip()
    ...
    def retrieve_context_from_knowledge_graph(self, query: str, top_k: int = 5) -> list:
        """Return raw Pinecone matches from the document namespace for ``query`` (not merged graph text)."""
    ...
    def retrieve_context_from_knowledge_graph(self, query: str, top_k: int = 5) -> list:
        """Resolve context for ``query`` by calling ``get_knowledge_graph_context`` with a one-item query list."""
...
def load_custom_transcript(path):
    """Load transcript lines from JSON at ``path`` into LangChain ``Document`` instances (module-level helper)."""
...
def obtainMetrics():
    """Compute conversational metrics from the global ``transcripts`` list (turns, words, balance score, interruptions)."""
...
def sentimentAnalysis(turn_counts):
    """Run VADER on global ``transcripts``, regress sentiment over time per speaker, and return summary strings."""
...
def speakerVolatility(speaker_sentiments):
    """Measure each speaker's sentiment swing (std dev of compound scores) and return a formatted summary string."""
    speaker_volatility = {}
...
def obtainBaseMetrics(segments):
    """Compute the same balance metrics as ``obtainMetrics`` from segment dicts (speaker, start, end, text)."""
    turn_counts = defaultdict(int)
    word_counts = defaultdict(int)
    interruptions = 0
...
def sentimentBaseAnalysis(segments, turn_counts):
    """Score ``segments`` with VADER, estimate per-speaker sentiment slopes, and normalize using ``turn_counts``."""
    from nltk.sentiment import SentimentIntensityAnalyzer
    from sklearn.linear_model import LinearRegression
    import numpy as np
...
def speakerBaseVolatility(speaker_sentiments):
    """Return per-speaker sentiment volatility (std dev of scores) for ``sentimentBaseAnalysis`` output."""
